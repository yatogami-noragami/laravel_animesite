    ...
    <div class="row mb-5">
        ...
        <div class="col-lg-5 col-md-6">
            ...
            <div class="row mt-3">
                ...
                <h5 class=" text-light mb-4">Status: <span class=" text-info">{{ $status }}</span></h5>
                <h5 class=" text-light mb-4">Total episodes: <span class=" text-info">{{ $anime->episode_count }}</span>
                </h5>
            </div>
                    ...
                    @if (in_array($anime->title, $animeNames))
                        <form action="{{ route('user#anime#bookmark#remove') }}" method="post">
                            ...
                            <button type="submit" class="btn btn-outline-danger form-control py-3 fs-5">
                                remove this anime from bookmark
                            </button>
                        </form>
                    @else
                        <form action="{{ route('user#anime#bookmark#add') }}" method="post">
                ...
                @foreach ($episodes as $episode)
                    <div class="col-3 col-md-2 col-lg-1 my-3">
                        <a href="{{ route('user#anime#watch', ['id' => $episode->anime_id, 'epid' => $episode->episode_number]) }}"
                            type="button" class="btn btn-outline-warning px-4">
                ...
                <div class="row">
                    @foreach ($recoms as $recom)
                        <div class="col-lg-4 col-md-6 col-6 ">
                            ...
                            <a href="{{ route('user#anime#details', $recom->id) }}" class=" text-decoration-none">
